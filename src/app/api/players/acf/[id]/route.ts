...
function parseACFProfile(html: string, acfId: string): PlayerData | null {
    try {
        const $ = cheerio.load(html)
        
        // Extract player name from the page title or main heading
        ...
        // Extract ratings from the stats box
        let stdRating: number | null = null
        let rapidRating: number | null = null // ACF calls this "QUICK"
        
        // Find the rating information in the stats box
        $('#stats-box #stats-box-data').each((index, element) => {
            const $row = $(element)
            const title = $row.find('#stats-box-col-title').text().trim()
            
            if (title === 'Current Rating:') {
                const $dataCols = $row.find('#stats-box-data-col')
                
                // First column is QUICK (rapid), second is STANDARD
                $dataCols.each((colIndex, col) => {
                    const ratingText = $(col).text().trim()
                    if (ratingText && ratingText !== 'None' && ratingText !== '') {
                        const rating = parseInt(ratingText)
                        if (!isNaN(rating)) {
                            if (colIndex === 0) {
                                rapidRating = rating // QUICK rating
                            } else if (colIndex === 1) {
                                stdRating = rating   // STANDARD rating
                            }
                        }
                    }
                })
        ...
        return {
            ...
            rapid_rating: rapidRating, // In ACF, this is "QUICK" rating
            blitz_rating: null         // ACF doesn't have blitz ratings
        }
    } catch (error) {
        console.error('Error parsing ACF profile:', error)
        return null
    }
}
