    ...
    try {
        ...
        const { data: updatedPricing, error: updateError } = await supabase
            ...
            .update({
                ...
                start_date,
                end_date,
                max_tickets: max_tickets ? parseInt(max_tickets) : null,
        ...
        if (updateError) {
            console.error('Error updating section pricing:', updateError)
            return NextResponse.json({ error: 'Failed to update pricing' }, { status: 500 })
        }

        return NextResponse.json(updatedPricing)
    } catch (error) {
        console.error('Error in section pricing PUT:', error)
