...
export default function MobileBookingModal({ event, user, isOpen, onClose, initialStep, resumeBookingId }: MobileBookingModalProps) {
    ...
    return (
        <div className="fixed inset-0 z-50 md:hidden">
            ...
            {/* Modal */}
            <div
                className={`fixed inset-x-0 bottom-0 bg-white dark:bg-gray-800 rounded-t-2xl shadow-2xl transition-transform duration-300 ease-out ${isAnimating ? 'transform translate-y-0' : 'transform translate-y-full'
                    }`}
                style={{
                    maxHeight: '90vh',
                    minHeight: '70vh'
            ...
            >
                {/* Header */}
                <div className="flex items-center justify-between p-4 border-b border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 rounded-t-2xl sticky top-0 z-10">
                    ...
                    <button
                        onClick={handleClose}
                        className="p-2 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-full transition-colors absolute right-4 top-4"
